...
\vspace*{-0.15cm}
L'estimateur du \textbf{groupe lasso} est: 
\[
...
\centering
\includegraphics[width=\textwidth]{apoe_interactions_sail_coefs}
...
\centering
\includegraphics[width=\textwidth]{apoe_interactions_mmscore_amy_beta_supramarginal_gyrus_right}
...
\centering
\includegraphics[width=\textwidth]{apoe_interactions_lasso_coefs}


## Comparaison des coefficients: \texttt{sail} vs. \texttt{lasso}

\includegraphics[width=0.5\textwidth]{apoe_interactions_sail_coefs}
\hfill
\includegraphics[width=0.5\textwidth]{apoe_interactions_lasso_coefs}


## Erreur quadratique moyenne de la validation croisée

\centering
\framedgraphic{apoe_interactions_5foldcv_mse_with_lasso_cropped}
...
\begin{itemize}
	...
	\item Erreur: \mbox{$\be = (\varepsilon_1, \ldots, \varepsilon_n) \in \mathbb{R}^{n}$} \pause
...
où $\bV = \eta \bPhi + (1-\eta) \bI$ \mbox{}$^{1}$  \pause 
\item Supposons que nous avons une matrice de rang inférieur $\mb{K} \in \mathbb{R}^{n\times k}$ ($k < n$), pour calculer la matrice de similarité factorisée $\bPhi = \mb{K}\mb{K}^T$ \pause
...
\item $\bbeta_{(k)}$ est la parti de $\bbeta$ qui correspond à la groupe $k$ \pause
...
\item Cependant, la fonction de perte $L(\bbeta|\bD)$ satisfait à la condition <<quadratic majorization>> (QM) $^1$, puiqu'il existe
\begin{itemize}
...
\begin{itemize}
\item Est-ce que les deux paramètres de réglage sont vraiment nécessaires ?
$$ 
